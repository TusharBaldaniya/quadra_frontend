const CACHE_NAME = 'quadra-cache-v3';
const PRECACHE_URLS = [
  '/',
  '/index.html',
  '/manifest.json',
...
self.addEventListener('push', (event) => {
  ...
  if (event.data) {
    try {
      ...
      data = { title: 'Quadra AI Coach', body: event.data.text() };
    }
  }

  const title = data.title || 'Quadra AI Coach 🎯';
  const options = {
    body: data.body || 'You have an active focus update!',
    icon: '/quadra-symbol-transparent.png',
    badge: '/quadra-symbol-transparent.png',
    vibrate: [100, 50, 100],
    data: {
      url: data.url || '/',
      logId: data.logId
